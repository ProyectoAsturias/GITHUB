...
function baseLayer(layerName) {
	...
	if (layerName == "OSM") {
		...
		$("#baseLayerButton #baseButton").empty().append(html);
		googleMapLayer(map);
	} else if (layerName == "GMS") {
		var html = "<span data-label-placement>Google Satellite</span> <span class=\"caret\"></span>";
		$("#baseLayerButton #baseButton").empty().append(html);
		googleMapSatelliteLayer(map);
...
function googleMapSatelliteLayer(map) {
	$("#gmap").addClass("fill");
	googleLayer = false;
	gmap = new google.maps.Map(document.getElementById('gmap'), {
			disableDefaultUI : true,
