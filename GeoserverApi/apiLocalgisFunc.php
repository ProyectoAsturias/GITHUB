<?php
	...
	function getEntityData(){
		if(isset($_POST['entityId'])){
			...
			$dbConnection = new DBConnection();
			$query = "SELECT replace(nombreoficial,' ','')as name,srid FROM entidad_supramunicipal WHERE id_entidad='".$entityId."'";
			$result = pg_query($query) or die('Error: '.pg_last_error());
			$numRows=pg_num_rows($result);
			while ($row = pg_fetch_array($result, null, PGSQL_ASSOC)) {
			...
			$query = "SELECT id_municipio FROM entidades_municipios WHERE id_entidad='".$entityId."'";
			$result = pg_query($query) or die('Error: '.pg_last_error());
			$numRows=pg_num_rows($result);
			while ($row = pg_fetch_array($result, null, PGSQL_ASSOC))
	...
	function getEntityNames(){
		$params=array();
		$dbConnection = new DBConnection();
		$query = "SELECT replace(nombreoficial,' ','')as name FROM entidad_supramunicipal";
		$result = pg_query($query) or die('Error: '.pg_last_error());
		$numRows=pg_num_rows($result);
		while ($row = pg_fetch_array($result, null, PGSQL_ASSOC)) {
			array_push($params,$row['name']);
